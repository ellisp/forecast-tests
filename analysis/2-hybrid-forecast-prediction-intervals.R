# test for a method of creating prediction intervals from an unweighted 
# forecast combination of ets() and auto.arima().
# Peter Ellis January 2016, originally discussed at http://ellisp.github.io/blog/2016/01/30/hybrid-forecasts/ 
...
# source for hybridf() - down the track I hope to merge this into {forecast}
# but there's some work to do yet eg accuracy- or CV-based weights
source("https://raw.githubusercontent.com/ellisp/forecast/dev/R/hybridf.R")


svg("output/2-USAccDeaths.svg", 6, 7)
...
png("output/2-USAccDeaths.png", 600, 700, res = 100)
fc <- hybridf(USAccDeaths)
par(mfrow = c(3, 1), bty = "l")
plot(fc)
plot(fc$fc_ets)
...
svg("output/2-indiv-error.svg", 9, 7)
print(
results %>%
   gather(variable, value, -h) %>%
